...
class IndependentParticipant {
    ...
    try {
      ...
      if (result.hasStaleChildren) {
        ...
        printer.log(
          depth: depth,
          participant: name,
          message: '♥ DETECTED: Stale participant(s): [$staleList] - crash detected! Ages: $staleAges',
        );
        _signalFailure(FailureDetection(
  ...
  Future<void> crash({required int depth}) async {
    printer.log(depth: depth, participant: name, message: '💥 CRASH (stopping heartbeat, hanging indefinitely...)');
    _isCrashed = true;
    _heartbeatTimer?.cancel();
    _heartbeatTimer = null;
  ...
  Future<void> cleanupOnFailure({required int depth}) async {
    printer.log(depth: depth, participant: name, message: 'cleanup on failure detection');
...
class ConcurrentScenarioRunner {
  ...
  Future<ConcurrentScenarioResult> runCrashDetectionScenario({
    ...
    try {
      ...
      _printer.log(depth: 0, participant: 'Runner', message: 'Crash target: $crashingParticipant after ${crashAfterMs}ms');
      _printer.log(depth: 0, participant: 'Runner', message: 'Heartbeat interval: ${heartbeatIntervalMs}ms, timeout: ${heartbeatTimeoutMs}ms');
      ...
      await cli.startCallExecution(callId: 'cli-main', depth: 1);
      ...
      await bridge.startCallExecution(callId: 'bridge-process', depth: 2);
      ...
      _printer.log(depth: 0, participant: 'Runner', message: 'Waiting for failure detection (timeout: ${maxWaitMs}ms)...');
  ...
  Future<ConcurrentScenarioResult> runHappyPath({
    ...
    try {
      ...
      await bridge.endCallExecution(callId: 'bridge-process', depth: 2);
      
      // CLI completes
      events.add(ScenarioEvent(stopwatch.elapsedMilliseconds, 'CLI completes'));
      cli.stopHeartbeat(depth: 1);
      await cli.endCallExecution(callId: 'cli-main', depth: 1);
  ...
  Future<ConcurrentScenarioResult> runAbortScenario({
    ...
    try {
      ...
      _printer.log(depth: 0, participant: 'Runner', message: 'Abort after ${abortAfterMs}ms');
      ...
      _printer.log(depth: 0, participant: 'Runner', message: 'Waiting for abort detection...');
