...
void main() {
  ...
  setUp(() async {
    ledgerPath = '${Directory.systemTemp.path}/concurrent_test_${DateTime.now().millisecondsSinceEpoch}';
  });
  ...
  group('ConcurrentScenarioRunner', () {
    ...
    test('detects Bridge crash through stale heartbeat', () async {
      ...
      expect(result.success, isTrue, reason: result.errorMessage ?? 'Unknown error');
      expect(result.detectedFailure, isNotNull);
      expect(result.detectedFailure!.type, equals(DetectedFailureType.staleHeartbeat));
    ...
    test('detects user abort through abort flag', () async {
      ...
      expect(result.success, isTrue, reason: result.errorMessage ?? 'Unknown error');
      expect(result.detectedFailure, isNotNull);
      expect(result.detectedFailure!.type, equals(DetectedFailureType.abortRequested));
    ...
    test('crash detection timing is realistic', () async {
      ...
      final expectedMinMs = 2500; // At least past crash time + some threshold time
